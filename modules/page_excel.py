...
def render_page_excel(api_key, anim_class):
    ...
    with col1:
        with st.container(border=True):
            ...
            if pdf_file_gen:
                # Bug #5 Fix: ใช้ key แยกต่างหาก ไม่ทับ cache ของ DATA AUDITOR
                ...
                st.markdown(f'<div style="font-size:12px;color:#3b82f6;font-weight:600;margin-top:8px;">CACHED: {st.session_state.cached_gen_pdf_name}</div>', unsafe_allow_html=True)


    with col2:
        with st.container(border=True):
            st.markdown('<div class="custom-card-marker"></div>', unsafe_allow_html=True)
            st.markdown(f'<div class="{anim_class} anim-delay-2"><div class="c-eye">STEP 2 (OPTIONAL)</div><div class="c-ttl" style="margin-bottom:4px;">Reference Excel</div></div>', unsafe_allow_html=True)
            ...
            if excel_ref_file:
                ...
                st.markdown(f'<div style="font-size:12px;color:#3b82f6;font-weight:600;margin-top:8px;">CACHED: {st.session_state.cached_gen_excel_name}</div>', unsafe_allow_html=True)
    ...
    with btn_col:
        if st.button("Generate Upload Excel from PDF", type="primary", use_container_width=True, disabled=not gen_ready):
            with st.spinner("AI is analyzing and generating Excel..."):
                try:
                    pdf_bytes_for_gen = pdf_file_gen.getvalue() if pdf_file_gen else st.session_state.get("cached_gen_pdf_bytes")
                    excel_bytes_for_gen = excel_ref_file.getvalue() if excel_ref_file else st.session_state.get("cached_gen_excel_bytes")
